...
CLI 入口（python3 -m installer）：
  ...
  python3 -m installer --svn <target>                      # SVN 团队治理初始化
  python3 -m installer --lightweight <target>              # 轻量治理 profile
"""
...
USAGE = ("用法: python3 -m installer [--update|--repair|--doctor|--uninstall|--dry-run] "
         ...
         "[--with-codebase-memory] [--svn] [--lightweight] /path/to/your-project")
...
class CliArgs:
    ...
    migrate_features: Optional[str] = None     # None = 未要求迁移
    sdd_layout: bool = False
    sdd_version: Optional[str] = None
    dry_run: bool = False
...
_FLAG_ACTIONS = {
    "--update":          lambda a: setattr(a, "mode", "install"),
    "--repair":          lambda a: setattr(a, "mode", "repair"),
    "--doctor":          lambda a: setattr(a, "mode", "doctor"),
    "--uninstall":       lambda a: setattr(a, "mode", "uninstall"),
    "--adopt-legacy":    lambda a: setattr(a, "mode", "adopt-legacy"),
    "--dry-run":         lambda a: setattr(a, "dry_run", True),
    "--sdd-layout":       lambda a: setattr(a, "sdd_layout", True),
    "--with-codebase-memory": lambda a: setattr(a, "with_codebase_memory", True),
    "--svn":             lambda a: setattr(a, "with_svn", True),
    "--lightweight":     lambda a: setattr(a, "lightweight", True),
}
...
def _parse_args(argv: list[str]) -> CliArgs:
    ...
    while args:
        arg = args.pop(0)
        _parse_one(arg, args, a)
    return a


def _parse_one(arg: str, rest: list[str], a: CliArgs) -> None:
    if arg in _FLAG_ACTIONS:
        _set_mode(a, arg)
        _FLAG_ACTIONS[arg](a)
        return
    if arg in _RECOVER_MODES:
        _set_mode(a, arg)
        a.mode = "recover"
        a.recover_strategy = _RECOVER_MODES[arg]
        return
    if arg == "--recover":
        _set_mode(a, arg)
        a.mode = "recover"
        ...
        s = rest.pop(0)
        if s not in ("rollback", "roll-forward"):
            sys.exit(f"错误: --recover 只接受 rollback|roll-forward（收到 {s!r}）")
        a.recover_strategy = s
        return
    if arg == "--migrate-specify":
        _set_mode(a, arg)
        if a.migrate_features is None:
            a.migrate_features = ""
        return
    if arg.startswith("--migrate-specify="):
        _set_mode(a, arg)
        a.migrate_features = arg.split("=", 1)[1]
        if not a.migrate_features:
            sys.exit("错误: --migrate-specify= 需要至少一个 feature 名（逗号分隔）")
        return
    if arg in _ARG_ACTIONS:
        attr, what = _ARG_ACTIONS[arg]
        if not rest:
            sys.exit(f"错误: {arg} 需要{what}")
        setattr(a, attr, rest.pop(0))
        return
    if arg.startswith("-"):
...
def _set_mode(a: CliArgs, arg: str) -> None:
    """模式互斥检查——同名可重复（如 --update --with-codebase-memory），异名报错。"""
    requested = (arg.removeprefix("--").split("=")[0]
                 if arg.startswith("--") else arg)
    mode_map = {"update": "install", "repair": "repair", "doctor": "doctor",
...
def _validate(a: CliArgs) -> Path:
    ...
    checks = [
        (a.dry_run and a.mode not in ("install", "repair"),
         "错误: --dry-run 仅用于安装方向"),
        (a.mode == "recover" and a.recover_strategy is None,
         "错误: --recover 需要 rollback 或 roll-forward"),
        (a.sdd_layout and not a.sdd_version,
         "错误: --sdd-layout 需要 --sdd-version <版本目录名>（如 V1.0）"),
        (a.sdd_version and not a.sdd_layout,
         "错误: --sdd-version 仅在 --sdd-layout 时使用"),
        (a.sdd_layout and a.migrate_features is None,
         "错误: --sdd-layout 仅在 --migrate-specify 时使用"),
        (a.with_codebase_memory and a.mode in ("uninstall", "doctor"),
         "错误: --with-codebase-memory 不能与 --uninstall/--doctor 同时使用"),
        (a.with_svn and a.mode not in ("install", "repair"),
         "错误: --svn 仅用于安装方向（治理初始化）"),
        (a.lightweight and a.mode not in ("install", "repair"),
    ...
    repo = Path(a.target).expanduser()
    if not repo.is_dir():
        sys.exit(f"错误: 目标目录不存在: {repo}")
    repo = repo.resolve()

    if a.migrate_features is not None and not (repo / ".specify" / "specs").is_dir():
...
def _dispatch_readonly(a: CliArgs, repo: Path, codex: Optional[Path]) -> Optional[int]:
    """doctor / uninstall / adopt-legacy / recover 独立分发；不匹配返回 None。"""
    if a.mode == "doctor":
    ...
    if a.mode == "recover":
        ...
        try:
            result = recover_manual(repo, a.recover_strategy)
        finally:
            os.close(lock_fd)
...
def main(argv: list[str] | None = None) -> int:
    _check_python()
    a = _parse_args(list(sys.argv[1:] if argv is None else argv))
    repo = _validate(a)
    codex = (Path(a.codex_root).expanduser().resolve()
             if a.codex_root is not None else None)

    rc = _dispatch_readonly(a, repo, codex)
    if rc is not None:
        return rc
    return _dispatch_install(a, repo, codex)


# ══════════════════════════ 旁路步骤（Spec 迁移 / 代码图谱 / SVN 治理） ══════════════════════════
...
def _post_install_extras(repo, migrate_features, sdd_layout, sdd_version,
    ...
    if (repo / ".specify" / "specs").is_dir():
        ...
        cmd += _spec_migrate_args(migrate_features, sdd_layout, sdd_version, repo)
        proc = subprocess.run(cmd)
