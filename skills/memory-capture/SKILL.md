...
先解析本 `SKILL.md` 的真实路径（跟随符号链接），将技能目录向上三级作为目标仓库根目录；确认该目录同时包含 `.repo-memory-kit/manifest` 与 `docs/memory/RULES.md`。后续所有相对路径和命令均以该目标仓库为准，不以 Codex 的 workspace/CWD 为准。若校验失败，再使用用户明确指定的仓库；仍无法唯一定位时先询问，禁止写入猜测的目录。
