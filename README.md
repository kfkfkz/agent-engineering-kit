...
跨仓库锚点已落地（协议 v2 的 `repo@` 键前缀；方法重载用参数签名消解——仓库与签名都是身份的一部分）。

**长期（信号驱动，不提前建设）**：

| 方向 | 触发信号 |
| --- | --- |
| 路线 A：记忆服务（自动捕获 + 蒸馏管线 + 混合检索） | 条目产量超过人工整理能力 |
| 路线 B：图谱原生边（fork 工具加 CITES + detect_changes 联动） | 锚点表撞到精度/联动天花板 |
| 检索升级：标签路由（50 条）→ sqlite-vec（200 条）→ 混合检索 | 对应条目数，或索引漏检 ≥ 2 次 |
