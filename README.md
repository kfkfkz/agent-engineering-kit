...
只要求用户给出确认/评审结论与签核人；确认后由 Agent 写记录、完成内部冻结并继续，不把
底层命令作为“你的下一步”。若必须在新任务或新会话继续，提示当前客户端的
...
3. **需求确认**：把"加个导出"衍生为具体场景（如"运营在订单列表页筛选后导出当日对账单"），每个场景定义可观察产出（谁在哪个页面拿到什么）；维护者确认当前版本和签核人后，Agent 写入签核记录、完成内部冻结并继续；导出方式、异步任务、文件存储等会改公共契约的细节一并交用户裁决。
...
计划阶段采用双层冻结：首次 PASS 保存**执行前计划基线**，执行期允许持续更新任务执行记录
和 checkbox，但任务定义、预计范围、验证方式发生变化仍会立即 DRAFT；交付门禁完成后由
Agent 统一 closeout，写入任务清单与测试方案的完整终态哈希。收口后的任何改动再次失效，
因此不需要每完成一个任务就重冻，也不会把正常执行事实误判成设计漂移。
...
由 Agent 写入评审记录、完成内部冻结并自动进入详细设计。下游发现上游不可实现走打回流程
