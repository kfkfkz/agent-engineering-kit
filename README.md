...
3. **需求确认**：导出方式、异步任务、文件存储等会改公共契约的细节交用户裁决；范围内细节直接推进。
4. **概要设计**：产出《订单列表导出-业务流程设计》——总体设计、业务闭环、功能划分与容量边界；需要新 CSV 组件时用 `reuse-research` 比较现有依赖、扩展点和新库。
5. **详细设计**：产出《订单列表导出-详细设计》——领域模型、导出契约、字段映射、数据库与接口、异常场景与验收要点，与概要设计互相引用；随后过 `delivery-gate` 文档门禁（结构/字段/一致性/可验证性），发现缺漏回补后落回执。
...
repo-delivery
  ...
  ├─ ② 需求确认：待澄清问题集中交人裁决
  ├─ ③ 概要设计：《功能名-业务流程设计》（总体设计/业务闭环/功能划分）
  ├─ ④ 详细设计：《功能名-详细设计》（领域模型/分侧设计/数据库/接口/验收）
