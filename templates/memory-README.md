...
5. 超限条目按「大小约束」压缩，并同步 README 索引与锚点表（按「锚点表」规则解析全部校验点符号、重写 `.anchors.json`）；`PROFILE.md`（L3，存在时）核对下钻链仍闭合并更新「最后蒸馏」日期。
...
1. **查锚点表**：变更的类/方法/文件/路由 → 查 `docs/memory/.anchors.json`，表命中即精确波及条目；表缺失或未命中时回退结构化检索文本反查（如 codebase-memory `search_code(符号, path_filter="docs/memory")`）。
...
- 结构：`anchors`（解析成功的符号 → 条目列表）+ `unresolved`（解析失败的符号、涉及条目与原因——**漂移或跨仓库信号**）。
- **刷新时机**：全量巡检时——解析全部条目「校验点」中的代码符号（结构化检索工具的符号查询），成功的进 `anchors`，失败的进 `unresolved`。
- `unresolved` 的处置不可静默丢弃：符号改名 → 更新条目校验点；确认跨仓库 → 标注原因保留。
