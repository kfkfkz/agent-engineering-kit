...
**前置依赖（默认强制）**：巡检必须使用结构化代码检索工具（代码知识图谱类，如 codebase-memory MCP；或提供符号级定位、调用关系、索引覆盖/新鲜度信号的等效 codegraph 工具）。grep 有三个结构性盲区，不能作为巡验兜底：服务路由前缀通常配置在 application 配置的 context-path（字面量搜不到）、类/方法改名后关键字静默失效、无索引覆盖信号时无法判断结果可信度。工具不可用时应报告缺失并终止巡检，除非用户明确要求降级（降级巡检的报告必须标注"grep 降级，置信度低"）。
