...
**系统状态知识的统一登记处**：项目已有业务域地图（或 SDD 等体系——按团队建议的格式组织、方便理解，**不是事实权威**）时，能力入口、调用链、域职责与「不负责」边界类知识统一登记在域地图，不在 playbook 里复制；**事实以代码为准**——域地图与代码冲突时修正域地图，不在记忆区另立口径。功能蒸馏中属于常驻能力描述的核心流程，路由为对应域地图条目的更新（经人确认后写入）。playbook 保留**跨域端到端串法**与场景化操作流程（环境访问、测试数据、临时任务）；流程中引用的单域细节（枚举值、校验规则、数据模型）以指针指向域地图，不展开复制。域地图条目同样遵守证据规则：从代码形成的结论必须带文件、类、方法或数据表证据。
...
5. 明确不吸收的：短期会话压缩、自动 L0 捕获、向量检索——属自动蒸馏阶段，人工体系引入只增维护成本。
